...
export default function App() {
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center min-h-full bg-neutral-950`}
    >
      <Navbar />
      <form
        onSubmit={handleSearch}
        className="bg-neutral-900 p-6 rounded-2xl shadow-lg md:w-200 space-y-4 mb-6 mt-10 border border-neutral-800"
        id="search-form"
      >
        ...
        />
        <button className="md:w-1/6 w-full bg-neutral-700 text-neutral-100 p-3 md:ml-1 rounded-lg hover:bg-neutral-600 active:bg-neutral-500 cursor-pointer transition-all duration-200 font-medium shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-neutral-600 focus:ring-opacity-50">
          Consultar
        </button>
      </form>
        ...
        <div className="space-y-4 md:hidden">
          {resultados.length > 0 ? (
            resultados.map((r) => (
              <div
                key={r.id}
                className="border border-neutral-800 rounded-lg p-4 shadow-sm bg-neutral-800"
              >
